...
import { ArrowRight, CalendarDays, Copy, Sparkles } from "lucide-react";
import { AppHeader } from "@/components/app-header";
import { GhostLink, Panel, Shell } from "@/components/ui";
import { getLocalUser, listDiaries } from "@/lib/local-diary";
    ...
    <Shell>
      ...
      <div className="grid flex-1 gap-5 lg:grid-cols-[1fr_280px]">
        <Panel className="book-cover flex flex-col justify-center overflow-hidden p-8 sm:p-12">
          <h2 className="max-w-3xl font-serif text-4xl font-bold leading-[1.08] text-primary sm:text-6xl">
            <span className="block">把一张照片</span>
            <span className="mt-3 block pl-0 text-secondary sm:pl-16">慢慢说成一篇日记</span>
          </h2>
        ...
        <aside className="space-y-5">
          <Panel className="p-5">
            ...
            <div className="mt-3 flex items-center justify-between gap-3">
              <p className="font-serif text-2xl text-secondary">{inviteCode}</p>
              <button
                aria-label="复制邀请码"
                className="inline-flex size-11 items-center justify-center rounded-xl border border-white/10 text-secondary transition hover:border-secondary/40"
            ...
            {copied ? <p className="mt-3 text-xs text-tertiary">已复制</p> : null}
          </Panel>
          ...
          <Panel className="p-5">
            ...
            {recentDiary ? (
              <Link className="mt-3 block rounded-2xl border border-white/10 bg-surface-dim p-3 transition hover:border-secondary/35" href={`/diaries/${recentDiary.id}`}>
                ...
                <img alt="最近日记封面" className="h-28 w-full rounded-xl object-cover" src={recentDiary.imageUrl} />
